...
!!! add the openacc 
 ...
 !! !$acc end parallel loop !!end loop for acc
  ...
  subroutine xtend(tendency,flux,ref,atmostat,dx,dt)
   ...
   !!  !$acc parallel loop collapse(2) present(atmostat%mem, flux%mem, ref%density, ref%denstheta) private(i,k,ll,s,stencil,vals,d3_vals,r,u,w,t,p)
    do k = 1, nz
      do i = 1, nx+1
        do ll = 1, NVARS
          do s = 1, STEN_SIZE
 ...
 !! !$acc end parallel loop
  subroutine ztend(tendency,flux,ref,atmostat,dz,dt)
    ...
    !!!!
   !!!    !$acc parallel loop collapse(2) present(atmostat%mem, flux%mem, ref%idens, ref%idenstheta, ref%pressure) private(i,k,ll,s,stencil,vals,d3_vals,r,u,w,t,p)
    do k = 1, nz+1
      do i = 1, nx
        do ll = 1, NVARS
          do s = 1, STEN_SIZE
  ...
  !!!  !$acc end parallel loop !!end acc loop
